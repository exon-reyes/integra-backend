 ...
 */
public interface UnidadRepository extends JpaRepository<UnidadEntity, Integer>,UnidadQueryRepository {
    /**
     ...
     */
    @Query("select (count(u) > 0) from UnidadEntity u where u.codigoAutorizacionKiosco = ?1 and u.id=?2")
    boolean existeCodigo(String codigoResetKiosco, Integer unidadId);
