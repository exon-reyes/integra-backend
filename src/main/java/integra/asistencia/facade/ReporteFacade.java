...
public class ReporteFacade {
    ...
    public ResumenMesAsistencia obtenerResumenMes(Integer empleadoId, int year, int month) {
        return asistenciaService.getResumenMes(empleadoId, year, month);
    }

    public List<LocalDate> obtenerDiasLaboradosEnMes(Integer empleadoId, int year, int month) {
        return asistenciaService.getDiasLaboradosEnMes(empleadoId, year, month);
    }
}